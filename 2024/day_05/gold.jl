...
text = read("example_1.txt", String)
lines = split(text, "\n")[begin:end-1]
...
#println(sum(map(update -> update[Int((end/2)+0.5)], bad_updates)))

sorted_priorities = []

for key in keys(rules_dict)
  last_index = 1
  max_size = 0
  for (idx,_) in enumerate(sorted_priorities)
    if length(intersect(Set(sorted_priorities),rules_dict[key])) > max_size
      max_size = length(intersect(Set(sorted_priorities),rules_dict[key]))
      last_index = idx
    end
  end
  global sorted_priorities = insert!(sorted_priorities, last_index, key)
end

bad_updates = filter(update -> !check_update(update,rules_dict), updates)

println(sorted_priorities)

test = map(update -> indexin(sorted_priorities, update),bad_updates)
